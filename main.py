...
def load_users():
    ...
    if os.path.exists(USERS_FILE):
        try:
            ...
            print("⚠️ ملف المستخدمين تالف أو فارغ — سيتم إعادة إنشاؤه.")
            return {}
    return {}
...
def save_users(users):
    ...
    try:
        ...
        print(f"❌ خطأ أثناء حفظ المستخدمين: {e}")

def track_user_and_notify_admin(m):
    """تتبع المستخدمين الجدد + إعلام الأدمن"""
    users = load_users()
    ...
    if user_id not in users:
        ...
        try:
            bot.send_message(
                ...
                f"👤 الاسم: {full_name}\n"
                f"🔹 اليوزر: {username}\n"
                f"🕒 وقت الانضمام: {users[user_id]['joined_at']}\n"
                f"📊 العدد الكلي للمستخدمين: {total_users}",
                parse_mode="HTML"
            )
        except Exception as e:
            print(f"[ERROR] فشل إرسال إشعار الأدمن: {e}")
...
BACKUP_INTERVAL = 60 * 60 * 1  # كل ساعتين
MAX_BACKUPS = 3  # الاحتفاظ بآخر 3 نسخ فقط
ADMIN_ID = 8038053114  # ضع هنا ID الإدمن

# الملفات الأساسية
USER_FILE = "user_numbers.json"   # أرقام المستخدمين
USERS_FILE = "bot_users.json"     # بيانات المستخدمين (الاسم، اليوزر...)
...
def create_backup():
    try:
        ...
        # تحميل بيانات المستخدمين والأرقام
        try:
            with open(USER_FILE, "r", encoding="utf-8") as f:
                user_numbers = json.load(f)
        except Exception:
        ...
        backup_data = {
            ...
            "user_numbers": user_numbers,  # 🔢 الأرقام
            "bot_users": bot_users,        # 👤 بيانات المستخدمين
...
def handle_backup_upload(message):
    ...
    try:
        ...
        user_numbers = backup_data.get("user_numbers", {})  # الأرقام
        bot_users = backup_data.get("bot_users", {})        # بيانات المستخدمين

        # حفظ الملفات بعد الاسترجاع
        with open(USER_FILE, "w", encoding="utf-8") as f:
        ...
        bot.reply_to(message, "✅ تم استرجاع النسخة الاحتياطية بالكامل بنجاح.")
    except Exception as e:
